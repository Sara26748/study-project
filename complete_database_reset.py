"""
Complete Database Reset for Versioning System

This script completely resets the requirement and requirement_version tables
and migrates all data from the project JSON blobs.
"""
...
def reset_and_migrate(db_path):
    """Reset tables and migrate data."""
    ...
    try:
        ...
        print("Dropping requirement_version table...")
        cursor.execute("DROP TABLE IF EXISTS requirement_version")
        
        print("Dropping requirement table...")
        cursor.execute("DROP TABLE IF EXISTS requirement")
        
        # Step 2: Create new requirement table
        ...
        # Step 3: Create new requirement_version table
        print("Creating new requirement_version table...")
        cursor.execute("""
        ...
        # Step 4: Get project data to read JSON blobs
        ...
        # Step 5: Migrate data from JSON blobs
        ...
        for project_id, created_json, intermediate_json, saved_json, deleted_json in projects_data:
            ...
            if not all_reqs:
                print(f"   Project {project_id}: No requirements found")
                continue
            
            print(f"   Project {project_id}: Found {len(all_reqs)} requirements")
            
            # Use dict to handle duplicates by key
            unique_reqs = {}
            ...
            print(f"   Project {project_id}: Migrating {len(unique_reqs)} unique requirements")
            ...
            for key, req_data in unique_reqs.items():
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
                """, (req_id, 1, version_label(1), title, description, category, status))
                
                total_migrated += 1
        
        conn.commit()
        print(f"Successfully migrated {total_migrated} requirements!")
...
def main():
    ...
    print("WARNING: This will completely reset requirement tables.")
    print("All existing requirements and versions will be deleted and recreated from project JSON data.")
    ...
    # Reset and migrate
    success = reset_and_migrate(db_path)
    ...
    if success:
        ...
        print("4. Generate new requirements to test Version B")
